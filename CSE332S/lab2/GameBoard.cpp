...
int board_dimensions_read(ifstream & input, unsigned & width, unsigned & height)
{
...
int game_pieces_read(ifstream & input, vector<game_piece> & positions, unsigned width, unsigned height)
{
...
int piece_print_out(const vector<game_piece> & pieces, unsigned width, unsigned height)
{
	...
	{
		cout << endl;
		for (int rowPrinting = (height - 1); rowPrinting >= -1; --rowPrinting)
		{
			if (rowPrinting == -1)
			{
				cout << "X";
